...
// Bundled through Vite so the URL is emitted into the build output and resolves
// correctly on any host (Lovable preview, Cloudflare, custom base paths).
import talaashLogoUrl from "@/assets/talaash-logo.png";
...
// Spacing reduced 25% from 55 -> 41.25 (still wide enough that no two
...
// Two streetside depth rows. Near row is capped low so rooflines never lean
// into the centre of frame where the road ahead / billboards are read.
const BUILDING_ROWS = [
  { base: BUILDING_MIN_CLEAR, cap: 9, tint: 0.0 },
  { base: BUILDING_MIN_CLEAR + 9, cap: 18, tint: 0.5 },
];
...
function Skyline() {
  const { boxes, crowns, litWins, darkWins, mullions } = useMemo(() => {
    ...
    const glassTone = new THREE.Color("#1b3a72");
    const podiumTone = new THREE.Color("#101c40");
    const deep = new THREE.Color("#05091c");
    const darkGlass = "#132a55";
    ...
    BUILDING_ROWS.forEach((row, r) => {
      [-1, 1].forEach((side) => {
        ...
        while (z > endZ) {
          k++;
          const d = 4.5 + rng(k + 300) * 5.5; // depth along the road
          const w = 3.2 + rng(k + 200) * 4.2; // footprint away from the road
          const prog = Math.min(1, Math.abs(z) / (TOTAL_LENGTH * 0.75));
          const h = Math.min(
            row.cap,
            (3 + rng(k + 100) * 14) * (0.45 + prog * 0.8),
          );
          const inner = row.base + rng(k + 700) * 0.5; // road-facing face x
          const zc = z - d / 2;
          const glass = h > row.cap * 0.55;
          const tone = (glass ? glassTone : podiumTone).clone();
          tone.lerp(deep, 0.35 + row.tint * 0.4 - (glass ? 0.1 : 0));
          ...
          tiers.forEach((tier, ti) => {
            ...
            const dens = r === 0 ? 1.15 : 1.7; // coarser grid on the far row
            const cols = Math.max(2, Math.floor(td / dens));
            const rowsN = Math.max(2, Math.floor(th / (r === 0 ? 1.05 : 1.6)));
...
function Ramp() {
  ...
  return (
    ...
    <group position={[0, 0, RAMP_Z + RAMP_LEN]}>
      ...
      {/* Crest wall behind the lip — occludes the final billboard on approach */}
      <mesh position={[0, RAMP_H + 1.5, -RAMP_LEN - 0.35]}>
        <boxGeometry args={[RAMP_W + 0.2, 3, 0.5]} />
        <meshStandardMaterial
          color="#0b1330"
          metalness={0.5}
          roughness={0.4}
      ...
      <mesh position={[0, RAMP_H + 3.02, -RAMP_LEN - 0.35]}>
        <boxGeometry args={[RAMP_W + 0.24, 0.1, 0.56]} />
        <meshBasicMaterial color={NEON_SOFT} toneMapped={false} />
      </mesh>
